// Vercel Function — enquiry / application email sender
// (migrated from netlify/functions/send-enquiry.js)
//
...
module.exports = async (req, res) => {
  ...
  if (!RESEND_KEY) return json(res, 500, { error: 'Email service not configured' });

  // Only accept submissions from our own site.
  const origin = req.headers.origin;
  if (origin && !ALLOWED_ORIGINS.includes(origin)) return json(res, 403, { error: 'Forbidden' });
  ...
  if (data.type === 'contact') {
    ...
    if (!fname && !lname) return json(res, 400, { error: 'A name is required' });
    ...
    try {
      ...
      if (!resendRes.ok) { console.error('Resend contact failed:', resendRes.status); return json(res, 502, { error: 'Could not send email' }); }
